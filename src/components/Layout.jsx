...
const hisSubItems = [
  { path: '/patient-journey', label: 'Patient Journey', emoji: '🏥', num: 1 },
...
const hisExampleSubItems = [
  { path: '/his-example/intro', label: 'HIS คืออะไร?', emoji: '💡', num: 0 },
...
// Shared sub-item NavLink renderer
function SubNavLink({ item, accentColor }) {
    ...
    >
      {isActive && (
        <div style={{
          ...
          width: 2, borderRadius: 2, background: accentColor,
        }} />
      )}
      ...
      {item.num > 0 && (
        <span style={{
          ...
          color: isActive ? accentColor : 'rgba(255,255,255,0.2)', flexShrink: 0,
        }}>{item.num}</span>
      )}
    </NavLink>
  )
...
export default function Layout({ children }) {
  ...
  const [hisExampleOpen, setHisExampleOpen] = useState(false)

  const isHisPage = ['/patient-journey', '/registration', '/opd', '/lab', '/pharmacy', '/ipd', '/finance', '/integration'].includes(location.pathname)
      ...
      <nav style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            fontSize: collapsed ? 18 : 22, color: '#fff',
          ...
          {!collapsed && (
            <div style={{
              ...
              fontFamily: "'JetBrains Mono', monospace", paddingLeft: 46,
            }}>
        ...
        <div style={{ flex: 1, padding: '0 8px', display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          {/* ── 1. HIS (formerly HIS Example) — FIRST ── */}
          <ExpandableSection
            id="his-example"
            label="HIS"
            emoji="🖥️"
            isActive={isHisExamplePage}
            accentColor="#9b59b6"
            open={hisExampleOpen}
            onToggle={() => setHisExampleOpen(!hisExampleOpen)}
            collapsed={collapsed}
            subItems={hisExampleSubItems}
          />

          <div style={{ height: 1, background: 'rgba(255,255,255,0.06)', margin: '6px 4px', flexShrink: 0 }} />

          {/* ── 2. กระบวนการรักษา (formerly HIS Modules) ── */}
          <ExpandableSection
            id="his"
            label="กระบวนการรักษา"
            emoji="🏥"
            isActive={isHisPage}
            accentColor="#667eea"
            open={hisOpen}
            onToggle={() => setHisOpen(!hisOpen)}
            collapsed={collapsed}
        ...
        <button
          ...
          style={{
            margin: '8px 12px', padding: '8px', flexShrink: 0,
...
function ExpandableSection({ label, emoji, isActive, accentColor, open, onToggle, collapsed, subItems }) {
  return (
    <div>
      <button
      ...
      {!collapsed && open && (
        <div style={{
          ...
          {subItems.map((item) => (
            <SubNavLink key={item.path} item={item} accentColor={accentColor} />
          ))}
        </div>
      )}
    </div>
